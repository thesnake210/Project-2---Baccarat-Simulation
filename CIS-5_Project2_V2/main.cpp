...
//Function Prototypes
unsigned short randCrd(); //Function to randomize card #
...
void bPtCalc(unsigned short);
bool pPairs(unsigned short);
bool bPairs(unsigned short);
...
int main(int argc, char** argv) {
    ...
    do {
        cards[crdSIZE] = randCrd(); //Randomization with max of 14 and min of 2 using function
        suitVal[crdSIZE] = rndSuit(crdSIZE); //Randomization to generate 1-4 for suit interpretation
        suits[crdSIZE] = rndSuit2(suitVal, crdSIZE); //Generates corresponding suits as string array
        ...
        */
        ...
        view[crdSIZE] = viewCrd(cards, crdSIZE);
        ...
        //Determine whether to draw 3rd card
        thrdCrd(subTot, cards, view, suits, 2, 3, 5);
...
unsigned short randCrd() {
    unsigned short num = rand() % (14 + 1 - 2) + 2;
    return num;
}
...
void bPtCalc(unsigned short subTot[]) {
    int bTot;
...
bool pPairs(unsigned short cards[]) {
    bool pBool;
    (cards[0] == cards[1] || cards[1] == cards[4] || cards[1] == cards[4]) ? pBool = true : pBool = false; 
    return pBool;
}

bool bPairs(unsigned short cards[]) {
    bool bBool;
