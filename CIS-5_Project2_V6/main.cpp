...
string faceGen(unsigned short [], int); //Generates face cards as strings from corresponding numbers
...
void bublSrt(int [], const int); //Bubble Sort - repeatedly swaps adjacent elements if misordered
    ...
    int scores[SIZE];
        ...
        if (view == 'y' || 'Y') {
            ...
            for (int j=0; j<SIZE; j++) {
                ...
                cout << "Would you like to view scores in ascending order? (y/n)" << endl;
                cin >> asc;
                while (view!= 'y' && view!='n' && view!='Y' && view!='N') {
                    cout << "Please enter y/n\n"
                         << "Would you like to view scores in ascending order? (y/n)" << endl;
                    cin >> asc;
                }
                if (asc == 'y' || 'Y') {
                    bublSrt(scores, SIZE);
                    cout << "High Scores (Ascending):" << endl;
                    ...
                    cout << names[k] << " " << scores[k] << endl;
                        ...
                        if (scr=='y' || scr=='Y') {
                            ...
                            binSrch(scores, names, SIZE, sScr, index); //runs function to find presence and index of score entered                        
                        }
                        else if (scr=='N' || scr=='n'){
                            cout << "You entered \"No\"...\n"
                                 << "Goodbye\n";
...
string faceGen(unsigned short crds[], int x) {
    string face;
...
void bublSrt(int a[], const int n) {
...
bool binSrch(int a[], string b[], int n, int value, int index) {
    ...
    for (int i=0; i<n; i++) {
        if (a[i]==value) {
            found = true;
            index=i;
            cout << value << " at completion by " << b[i] << " in position " << n-index << endl;
        }
        else {
            found = false;
        }
